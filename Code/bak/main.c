...
static UINT8 charge_cnt = 0;   //充电电流测试次数
...
void test_flow(){
  ...
  // -----------------------------------------------RATE 充电电流测试
  Timer0_Delay1ms(4000); 	// 开机等待 7000->5000->3000->1000
  get_current_ok = 0;
...
-------*/
  ...
  if(~pass || tempV < 0x00c8){  // < 0.2A -- 如果当前没有pass(=0)，或电流值小于基准，NG抛出
 // if(~pass){                       //可能以上取数据超时
  	pass = 0; 				// 上次漏了这行
    do_a_judgement(); 
    return;   
  ...
  // 测试5次，有一次OK,即退出while,pass=1
  // 测试5次，全部为NG,while自动结束,pass=0
  /*
  while (1){
  	if (tempV > 0x00c8) {          // 电流OK
  		pass = 1; 
  		charge_cnt = 0; break;
  	}
  	else {
  	   if (charge_cnt > 15)	{    // 电流NG，最多测的次数 (共：0.35s * 15 =5.25s)
  	   	pass = 0;
  	   	charge_cnt = 0; break;
  	   }
  	  else{
  	  	// 再次测量电流
  		Timer0_Delay1ms(350);   // 电流表的采样率为每秒3次
...
*/ 
  // -----------------------------------------------SHORT 短路电流测试
														  ...
														  //  短路
  CHARGE_OFF;
  Timer0_Delay1ms(1000); 		 // 断开充电并延迟1s -> 0.5s  执行短路
  SHORT_ON;						  
  Timer0_Delay1ms(500); 	     //	 延迟0.5s -> 0.1 -> 0.5s  
  OK_ON; Timer0_Delay1ms(1000); OK_OFF;    //执行(1s -> 0.5s ->1s) 的OK信号输出。 
   while (P03);					// PLC收到短暂OK信号后，去开启电池；此时等待PLC执行开电池的动作信号
  Timer0_Delay1ms(2000); 	     //	延迟2s －> 1.5s(上次错成15s)->2s (约电池已经开启输出了）开始测电流
 ...
 */
  ...
  if(~pass || tempV > 0x0014){  // < 0.02A    (此处NG时间长短不作考虑)
...
//  do_a_judgement();				// 程序能走到此处，表示OK，输出OK信号
  Timer0_Delay1ms(1000);		// 1s -> 0.5s秒后，放开短路	
    SHORT_OFF;
  Timer0_Delay1ms(200);		// 开启短路后，延迟0.2秒，重新合上充电回路
	CHARGE_ON;				//做OK、NG的判后改变charge ON, 让输入有感知  		
  Timer0_Delay1ms(5000);		// 开启短路后，延迟0.2秒，重新合上充电回路
  do_a_judgement();				// 程序能走到此处，表示OK，输出OK信号
}
