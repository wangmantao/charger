...
static unsigned int DCV_BAT_DOWN = 273;        // battery (1v) (1/3v)/5v *4095 (因为取样分压1/3)--短路检验用
...
void ifBatOut(){
	if(adc_val > DCV_BAT && P13 == 1 ){	       //未短路状态下		//  放电的下降值，和电池的检测值 ，用同样的4.5V。p13=CHARGE 常闭打开，不充电
...
void ifBatDown(){
	if(adc_val < DCV_BAT_DOWN && P13 == 1 ){	       //短路状态下, 电池电压1V以下		
		if(adc_count > 50000){					// 1.5秒持续高
			adc_count = 0;
			bat_down_ok = 1;
			clr_ADCEN;				// 	开始测试，停用ADC
...
void test_flow(){
	...
	Timer0_Delay1ms(1000); 	// 初次等待1s
  	// 循环测电流
	test_couter = 0;
	while( pass && test_couter < 35){			// 有一项为0 ，即为0（与）35次，约测10秒 
	  get_current_ok = 0;
	  timer0_monitor_start();
	  send_cmd();
	  while(~get_current_ok && ~time_out);	// 即没得到电流，又没超时一直等
	  timer0_monitor_stop();
	  if(time_out)
	  	pass= 0;
	  if(get_current_ok)     				// 取得电流
	  	tempV = current_val[5]; tempV <<= 8; tempV |= (current_val[4] & 0xFF);
	  if (tempV > 0x00c8)           
	    break;								// 如果电流值输出OK，保证不再为电流赋新值
	  else
	  	test_couter ++;
  ...
  // -----------------------------------------------SHORT 短路电流测试
  CHARGE_OFF;
  Timer0_Delay1ms(1500); 		 // 断开充电并延迟0.6s -> 1.5s

	/* 方案1 */
  /*
  SHORT_ON;						 // 执行放电与短路 -> 移至获得信号后
  OK_ON; Timer0_Delay1ms(1000); OK_OFF; // 发出OK指示 1s
   while (P03);
   Timer0_Delay1ms(2000); 	     //	延迟2s 
*/
	/* 方案2 */
	// 放开充电->给出1秒OK信号(同时放电0.5秒)->得到回复后->检验电池电压升起->执行短路->检验电池电压下降 -> 测电流	
  //SHORT_ON;	
  OK_ON;
  Timer0_Delay1ms(500);
  //SHORT_OFF;   // 放电0.5s
  ...
  while(~bat_out_ok);    //此时电压值 > 4.8V < 8V,持续1秒钟，表示BAT,OK

  SHORT_ON;             // 电池开启 －〉 shorted － 〉 电池电压下降 － 〉 测短路电流

  bat_down_ok = 0;    // 短路测试不等电压下降
  set_ADCEN; clr_ADCF; set_ADCS; set_ADCEN;
  while(~bat_down_ok);   //此时电压值 > 1V,持续1秒钟，表示短路有效

  Timer0_Delay1ms(1000);   //查看会不会显示"E"
